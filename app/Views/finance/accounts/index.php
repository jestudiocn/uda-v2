...
<div class="card">
    <form method="post" class="form-grid">
        ...
        <input id="account_type" type="text" name="account_type" placeholder="例如：现金、银行、电子钱包">
        <div class="form-full">
            <button type="submit" name="create_account" value="1">新增账户</button>
        </div>
    </form>
</div>
        ...
        <table class="data-table table-valign-middle">
            ...
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <td class="cell-tip"><?php echo html_cell_tip_content((string)($row['account_type'] ?? '')); ?></td>
                    <td><span class="chip"><?php echo ((int)($row['status'] ?? 0) === 1) ? '启用' : '停用'; ?></span></td>
                    <td>
                        <form method="post" action="/finance/accounts" style="display:inline;">
                            <input type="hidden" name="toggle_account" value="1">
                            <input type="hidden" name="account_id" value="<?php echo (int)($row['id'] ?? 0); ?>">
        ...
        <div class="toolbar" style="justify-content:space-between;">
            ...
            <div class="inline-actions">
                <?php for ($p = 1; $p <= (int)$totalPages; $p++): ?>
                    <a class="btn" style="<?php echo $p === (int)$page ? '' : 'background:#64748b;'; ?>padding:6px 10px;min-height:auto;" href="/finance/accounts?per_page=<?php echo (int)$perPage; ?>&page=<?php echo $p; ?>"><?php echo $p; ?></a>
                <?php endfor; ?>
            </div>
        </div>
    </div>
