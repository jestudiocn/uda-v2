...
<div class="card">
    <div class="toolbar" style="justify-content:space-between;">
        <h2 class="page-title">财务管理 / 待收款列表</h2>
        <a class="btn" href="/finance/receivables/create">新增待收款</a>
    </div>
    <form method="get" class="toolbar">
        <label for="status">状态</label>
        <select id="status" name="status">
            <option value="all" <?php echo $statusFilter === 'all' ? 'selected' : ''; ?>>全部</option>
            <option value="pending" <?php echo $statusFilter === 'pending' ? 'selected' : ''; ?>>待收款</option>
            <option value="received" <?php echo $statusFilter === 'received' ? 'selected' : ''; ?>>已收款</option>
        </select>
...
<div class="card">
    <div style="overflow:auto;">
        <table class="data-table">
            ...
            <tbody>
            <?php if (empty($rows)): ?>
                <tr><td colspan="9" class="muted">暂无待收款记录</td></tr>
            <?php else: ?>
                <?php foreach ($rows as $row): ?>
                    ...
                    <tr>
                        ...
                        <td class="cell-tip"><?php echo html_cell_tip_content((string)($row['expected_receive_date'] ?? '')); ?></td>
                        <td>
                            <span class="chip"><?php echo $status === 'received' ? '已收款' : '待收款'; ?></span>
                            <?php if ($dueLabel !== ''): ?>
                        ...
                        <td>
                            <?php if (trim((string)($row['voucher_path'] ?? '')) !== ''): ?>
                                <button type="button" class="btn js-finance-voucher-open" style="padding:6px 10px;min-height:auto;" data-url="/finance/voucher/view?kind=receivable&amp;id=<?php echo (int)($row['id'] ?? 0); ?>">查看</button>
                            <?php else: ?>
                        ...
                        <td>
                            <?php if ($status === 'pending'): ?>
                                <a class="btn" style="padding:6px 10px;min-height:auto;" href="/finance/receivables/settle?id=<?php echo (int)($row['id'] ?? 0); ?>">确认收款</a>
                            <?php else: ?>
        ...
        <div class="toolbar" style="justify-content:space-between;">
            ...
            <div class="inline-actions">
                <?php for ($p = 1; $p <= (int)$totalPages; $p++): ?>
                    <a class="btn" style="<?php echo $p === (int)$page ? '' : 'background:#64748b;'; ?>padding:6px 10px;min-height:auto;" href="/finance/receivables/list?status=<?php echo urlencode((string)$statusFilter); ?>&due_filter=<?php echo urlencode((string)$dueFilter); ?>&per_page=<?php echo (int)$perPage; ?>&page=<?php echo $p; ?>"><?php echo $p; ?></a>
                <?php endfor; ?>
            </div>
        </div>
    </div>
