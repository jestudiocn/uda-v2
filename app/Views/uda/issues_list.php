...
?>
<div class="card">
    <h2 style="margin:0 0 6px 0;">UDA快件 / 问题订单 / 问题订单列表</h2>
    <div class="muted">对应 V1 列表查询，列表风格比照派送订单查询。</div>
</div>

<?php if (!$schemaReady): ?>
    <div class="card" style="border-left:4px solid #dc2626;">问题订单相关表不存在，无法查询。</div>
    <?php return; ?>
<?php endif; ?>
...
<div class="card">
    <form method="get" style="display:grid;grid-template-columns:repeat(4,minmax(220px,1fr));gap:10px;align-items:end;">
        <div><label>面单号</label><input name="q_track" value="<?php echo htmlspecialchars($qTrack); ?>"></div>
        <div>
            <label>地点</label>
        ...
        <div><label>问题原因（可手输）</label><input name="q_reason_text" id="q_reason_text" value="<?php echo htmlspecialchars($qReasonText); ?>" placeholder="可留空"></div>
        <div>
            <label>是否已处理</label>
            <select name="q_processed">
                <option value="">全部</option>
                <option value="未处理" <?php echo $qProcessed === '未处理' ? 'selected' : ''; ?>>未处理</option>
                <option value="处理中" <?php echo $qProcessed === '处理中' ? 'selected' : ''; ?>>处理中</option>
                <option value="已处理" <?php echo $qProcessed === '已处理' ? 'selected' : ''; ?>>已处理</option>
            </select>
        </div>
        <div><label>创建日期（起）</label><input type="date" name="q_from" value="<?php echo htmlspecialchars($qFrom); ?>"></div>
        <div><label>创建日期（止）</label><input type="date" name="q_to" value="<?php echo htmlspecialchars($qTo); ?>"></div>
        <div class="inline-actions"><button type="submit">查询</button><a class="btn" href="/uda/issues/list">重置</a></div>
    </form>
</div>
...
<div class="card">
    ...
    <div style="overflow:auto;">
        <table class="data-table">
            <thead>
                <tr>
                    <th>面单号</th><th>地点</th><th>问题原因</th><th>处理方式</th><th>状态</th><th>创建日期</th><th>处理日期</th><th>备注说明</th><th>处理</th>
                </tr>
            </thead>
            <tbody>
            <?php if (empty($rows)): ?>
            ...
            <?php else: ?>
                <?php foreach ($rows as $r): ?>
                    ...
                    if (!in_array($statusText, ['未处理', '处理中', '已处理'], true)) $statusText = '未处理';
                    ...
                    <tr>
                        ...
                        <td><span class="<?php echo htmlspecialchars($statusCls); ?>"><?php echo htmlspecialchars($statusText); ?></span></td>
                        ...
                        <td>
                            <button type="button" class="btn issue-handle-btn" data-detail="<?php echo htmlspecialchars(json_encode($detailPayload, JSON_UNESCAPED_UNICODE), ENT_QUOTES, 'UTF-8'); ?>" style="padding:4px 8px;min-height:auto;">处理</button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <?php if ($totalPages > 1): ?>
        ...
        <div style="margin-top:10px;display:flex;gap:8px;">
            <?php if ($page > 1): $prev = $base; $prev['page']=(string)($page-1); ?><a class="btn" href="/uda/issues/list?<?php echo htmlspecialchars(http_build_query($prev)); ?>">上一页</a><?php endif; ?>
            <?php if ($page < $totalPages): $next = $base; $next['page']=(string)($page+1); ?><a class="btn" href="/uda/issues/list?<?php echo htmlspecialchars(http_build_query($next)); ?>">下一页</a><?php endif; ?>
        </div>
    <?php endif; ?>
</div>
...
<div id="issueHandleModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.35);z-index:9999;align-items:center;justify-content:center;padding:12px;">
    <div style="position:relative;max-width:900px;width:100%;max-height:92vh;overflow:auto;background:#fff;border-radius:12px;padding:16px 18px;box-shadow:0 10px 28px rgba(0,0,0,.2);">
        ...
        <h3 style="margin:0 0 10px 0;">问题订单处理</h3>
        <form method="post" class="form-grid" style="grid-template-columns:160px 1fr;gap:10px;">
            ...
            <label>面单号</label><div id="ih_tracking_no" style="padding:8px 10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">—</div>
            <label>地点</label><div id="ih_location_name" style="padding:8px 10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">—</div>
            <label>问题原因</label><div id="ih_problem_reason" style="padding:8px 10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">—</div>
            <label>处理方式（下拉）</label>
            ...
            <label>处理方式（可手输）</label><input name="handle_method_text" id="ih_handle_method_text" placeholder="可留空">
            <label>是否已处理</label>
            ...
            <label>备注说明</label><textarea name="remark" id="ih_remark" rows="4" placeholder="可修改"></textarea>
            <div class="form-full inline-actions"><button type="submit">保存处理</button></div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.issue-handle-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('ih_tracking_no').textContent = String(payload.tracking_no || '—');
            document.getElementById('ih_location_name').textContent = String(payload.location_name || '—');
            document.getElementById('ih_problem_reason').textContent = String(payload.problem_reason || '—');
